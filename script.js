...
function isRunningIOS() {
  return (
    iOSPlatformList.has(navigator.platform) ||
...
let itemsPerRowSlider,
  itemsPerRowLabel,
  itemNameInput,
  itemQuantityInput,
  itemPriceOrMultiplierInput,
  itemTable;
...
let settingsOverlay,
  ...
  includeSettingsInItemListCheckBox,
  copyCurrentItemsFromItemListCheckBox,
  abbreviationMappingTable,
  bottomTextSettingInput,
  textListSeparatorRadios,
  textListCustomSeparatorInput,
  textListSeparatorCustomRadio,
  textListFormatInput,
  priceCalculationItemInput,
  priceCalculationItemInputList,
  showPriceInScreenshotCheckBox,
  showTotalInNormalModeCheckBox,
  hidePriceOrMultiplierCheckBox,
  defaultQuantityInput,
  defaultPriceOrMultiplierInput,
  refocusNameOnSubmitCheckBox,
  focusQuantityOnAutocompleteCheckBox,
  ignoreLocaleCheckBox /*, reduceAnimationsCheckBox*/;
...
let totalSelectedPriceArea,
  totalSelectedPriceHolder,
  totalSelectedPriceMessageHolder,
  totalSelectedPriceEquationHolder;
let coinImageUrl;
let priceCalculationItem;
let priceCalculationItemList;
...
let shouldRefocusNameOnSubmit,
  shouldFocusQuantityOnAutocomplete,
  shouldIgnoreLocale;
...
$(document).ready(() => {
  ...
  priceCalculationItemInputList = $("#priceCalculationItemInputList");
  ...
  disableOutsidePriceCalculationModeElems = $(
    ".disableOutsidePriceCalculationMode"
  );
  ...
  priceCalculationItemFuzzyMatchesHolder = $(
    "#priceCalculationItemFuzzyMatchesHolder"
  );
  ...
  itemNameInput.on("keydown", (e) => {
    ...
    if (buttons.length < selection) return;
  ...
  $("#itemDeleteButton").on("click", (e) => {
    ...
    if (!formatItemName(itemNameInput.val()).length) {
      ...
      if (shouldRefocusNameOnSubmit) itemNameInput.trigger("select");
  ...
  importFileInput.on("change", async (event) => {
    ...
    if (files.length) importAll(await files[0].text());
  ...
  deleteItemListButton.on("click", () => {
    if (activeItemList === "Default") {
      console.log(
        "Attempted to delete Default item list!  This shouldn't be possible; please contact JJCUBER."
      );
      return;
    }
  ...
  createItemListButton.on("click", () => {
    if (itemLists.has(createItemListInput.val())) {
      console.log(
        "Attempted to create an existing item list!  This shouldn't be possible; please contact JJCUBER."
      );
      return;
    }
  ...
  textListSeparatorRadios
    .eq(textListSeparatorSelectedRadio)
    .prop("checked", true);
  textListCustomSeparatorInput[0].disabled =
    textListSeparatorRadios[textListSeparatorSelectedRadio].id !==
    "textListSeparatorCustomRadio";
  ...
  textListSeparatorRadios.each((i, elem) => {
    $(elem).on("click", i, (event) => {
      ...
      textListCustomSeparatorInput[0].disabled =
        event.target !== textListSeparatorCustomRadio[0];
  ...
  showPriceInScreenshotCheckBox.on("click", () => {
    ...
    if (!wasShowing) updateTotalPrice();
    saveAllToLocalStorage();
  });
  ...
  $("#clearAllButton").on("click", () => {
    if (
      confirm(
        "Are you sure?  This will clear the items currently added and can't be undone."
      )
    ) {
      items.clear();
      updateItemLayout();
  ...
  $("#selectAllButton").on("click", () => {
    ...
    if (shouldHideUnselectedItems) updateItemLayout();
  });
  ...
  $("#subtractSelectedQuantitiesButton").on("click", () => {
    for (let item of items.values()) {
      ...
      if (item.quantity <= 0) items.delete(item.name);
  ...
  $("#deleteSelectedButton").on("click", () => {
    for (let item of items.values()) {
      if (item.isSelected) items.delete(item.name);
    }
  ...
  $("#equationVisibilityToggleButton").on("click", () => {
    ...
    const toggleButton = event.currentTarget;
    if (wasHidden) toggleButton.classList.add("selected");
    else toggleButton.classList.remove("selected");
  });
  ...
  $("#unselectedItemsVisibilityToggleButton").on("click", () => {
    ...
    const toggleButton = event.currentTarget;
    if (wasHidden) toggleButton.classList.remove("selected");
    else toggleButton.classList.add("selected");
  ...
  if (isRunningIOS()) $("input, textarea").css("font-size", "16px");
...
const operators = [
  ...
  ">>>",
  ">>",
]; // >>> should be before >> to ensure the full operator gets removed then readded later (if >> was first, ">>> 5" would only remove the first 2 '>' leaving "> 5")
function handleAddingItem(e, usedSubmitButton = false) {
  ...
  if (itemNameInput.hasClass("invalid")) itemNameInput.removeClass("invalid");
  ...
  if (isActivelyCopyingImage) return;

  // TODO -- might want to be using e.key instead
  if (!usedSubmitButton && e.code !== "Enter") return;
  ...
  if (!itemNameFormatted.length) return;

  let itemQuantity,
    prependedQuantityOperator = "";
  ...
  if (items.has(itemNameFormatted)) {
    for (let operator of operators) {
      if (!itemQuantityEquation.startsWith(operator)) continue;

      prependedQuantityOperator = operator;
      itemQuantityEquation = itemQuantityEquation.substr(
        prependedQuantityOperator.length
      );
      break;
    }
  }
  ...
  try {
    ...
    itemQuantity = itemQuantityEquation.length
      ? math.evaluate(itemQuantityEquation)
      : 0;
  } catch (e) {
  ...
  addItem(
    itemNameFormatted,
    itemQuantity,
    itemPriceOrMultiplierInput.val(),
    prependedQuantityOperator
  )
...
class Item {
  ...
  calculateTotalPrice(shouldIgnoreCustomValues = false) {
    let quantity = this.customQuantity ?? this.quantity,
      priceOrMultiplier =
        this.customPriceOrMultiplier ?? this.priceOrMultiplier,
    ...
    if (!maxPrice && !customItemNames.has(this.name))
      return [
        NaN,
        "NaN",
        `${this.getHumanReadableName()} doesn't have a valid maximum price (${maxPrice}).`,
      ];
    ...
    if (!priceOrMultiplier.length) {
      ...
      if (!maxPrice)
        return [
          NaN,
          "NaN",
          `${this.getHumanReadableName()} is a custom item without a valid maximum price (${maxPrice}).`,
        ];
    ...
    else price = priceOrMultiplier;

    if (mult) price = `${maxPrice}*(${mult})`;
    ...
    try {
      ...
      return [
        NaN,
        price,
        `${this.getHumanReadableName()} has an invalid price/multiplier (price/multiplier: ${priceOrMultiplier} -- equation: ${price}).`,
        warning,
      ];
    }
  }
...
function addItem(
  ...
  if (!prependedQuantityOperator) itemQuantity = Math.floor(itemQuantity);
  ...
  if (items.has(itemNameFormatted)) {
    ...
    if (isInPriceCalculationMode) {
      if (prependedQuantityOperator.length)
        itemQuantity = Math.floor(
          math.evaluate(
            `${
              currItem.customQuantity ?? currItem.quantity
            } ${prependedQuantityOperator} ${itemQuantity}`
          )
        );
      ...
      currItem.customQuantity =
        itemQuantity > 0 && itemQuantity !== currItem.quantity
          ? itemQuantity
          : undefined;
      currItem.customPriceOrMultiplier =
        itemPriceOrMultiplier.trim().length &&
        itemPriceOrMultiplier !== currItem.priceOrMultiplier
          ? itemPriceOrMultiplier
          : undefined;
    ...
    if (prependedQuantityOperator.length)
      itemQuantity = Math.floor(
        math.evaluate(
          `${currItem.quantity} ${prependedQuantityOperator} ${itemQuantity}`
        )
      );
    ...
    if (itemQuantity > 0) {
      ...
      if (
        currItem.customQuantity === currItem.quantity ||
        (currItem.customQuantity > currItem.quantity &&
          currItem.customQuantity < prevQuantity)
      )
        // after updating an item's actual quantity, custom quantities above it should reset/"snap" back to the new quantity (unless it was already above it; custom quantity won't ever equal previous quantity, since it would have just gotten reset to undefined)
    ...
    } else items.delete(itemNameFormatted);
  ...
  if (isInPriceCalculationMode) return Promise.resolve();

  if (itemQuantity <= 0) return Promise.resolve();

  return getImageUrl(itemNameFormatted).then(async (imageUrl) => {
...
function formatItemName(itemName) {
  ...
  if (!itemNameTrimmed.length) return itemNameTrimmed;
...
function getImageUrl(itemNameTitleSnakeCase) {
  ...
  return fetch(
    `https://hayday.fandom.com/api.php?action=query&prop=imageinfo&iiprop=url&titles=File:${itemNameTitleSnakeCase}.png&format=json&origin=*`
  )
...
function updateItemLayout() {
  ...
  if (shouldShowSelection || getIsPriceShownInScreenshot()) updateTotalPrice();
  ...
  if (shouldShowSelection && shouldHideUnselectedItems) {
    ...
    if (!itemsUnsorted.length) return;
  }
  const itemsSortedDescending = itemsUnsorted.sort(
  ...
  while (rowCt--) {
    ...
    for (let j = 0; i < itemCt && j < itemsPerRow; i++, j++) {
      ...
      $(tableCell).on("mouseup", { index: i, item: currItem }, (event) => {
        ...
        itemPriceOrMultiplierInput.val(
          shouldShowSelection
            ? item.customPriceOrMultiplier ?? item.priceOrMultiplier
            : item.priceOrMultiplier
        );

        if (!shouldShowSelection) return;

        const index = event.data.index;
        const [first, last] = [index, previousSelection ?? index].sort(
          (n1, n2) => n1 - n2
        );
        if (event.shiftKey) {
          // range additive
          ...
          // range subtractive; must use if else if... to make sure previousSelection always gets set
          $("#itemTable td")
            .slice(first, last + 1)
            .each((k, elem) => {
              const currItem = itemsSortedDescending[first + k];
        ...
        } // normal selection (individual toggle)
        else {
      ...
      $(image).on("click", () => {
        ...
        if (!shouldShowSelection) itemNameInput.trigger("select");
        ...
        else if (shouldHideUnselectedItems) updateItemLayout();
      });
      ...
      if (shouldHidePriceOrMultiplier) priceLabel.style.display = "none";
      ...
      if (shouldShowSelection) {
        ...
        if (currItem.customQuantity !== undefined) {
          ...
          customQuantityLabel.classList.add(
            "label",
            "customLabel",
            "customQuantityLabel"
          );
          ...
          if (currItem.isSelected) quantityLabel.style.opacity = 0.5;
        }

        if (currItem.customPriceOrMultiplier !== undefined) {
          customPriceLabel = document.createElement("p");
          customPriceLabel.innerHTML = formatItemPriceLabel(
            currItem.customPriceOrMultiplier
          ); // using innerHTML so that coin image is shown
          customPriceLabel.classList.add(
            "label",
            "customLabel",
            "customPriceLabel"
          );
          ...
          if (currItem.isSelected) priceLabel.style.opacity = 0.5;
        }
      }
      ...
      if (customQuantityLabel) tableCell.appendChild(customQuantityLabel);
      if (customPriceLabel) tableCell.appendChild(customPriceLabel);
...
function formatItemPriceLabel(priceOrMultiplier) {
  ...
  return (
    priceOrMultiplier +
    (!priceStrTrimmed.length || priceStrTrimmed.endsWith("x")
      ? ""
      : `<img class="coin" src="${coinImageUrl}">`)
  );
}
...
function copyImageToClipboard() {
  ...
  if (!$(".watermark").length)
    // only append if the watermark always visible on screen didn't get removed
  ...
  if (isRunningIOS()) {
    ...
    clipboardWrittenPromise = htmlToImage
      .toBlob(screenshotRegion[0])
  ...
  clipboardWrittenPromise
    ...
    .catch((e) => {
      ...
      if (screenshotBlob) {
        failedCopyOverlay.imageHolder[0].src =
          window.URL.createObjectURL(screenshotBlob);
...
function copyAsTextListToClipboard() {
  ...
  const itemsSortedDescending = [...items.values()].sort(
    (item1, item2) => item2.quantity - item1.quantity
  );
  for (let item of itemsSortedDescending)
    itemStrs.push(formatTextListItem(format, item));

  const textList = itemStrs.join(
    textListSeparatorRadios[textListSeparatorSelectedRadio].value
  );
  navigator.clipboard
    .writeText(textList)
    .then(createSuccessfulCopyNotification)
...
function getMaxPrice(itemNameTitleSnakeCase) {
  // the custom item names get mapped back to themselves when getting the title snake case list, so they are in their "normal" form already at this point
  if (customItemNames.has(itemNameTitleSnakeCase)) return Promise.resolve(NaN);

  const properPageName = handleSpecialPageNames(itemNameTitleSnakeCase);
  const url =
    "https://hayday.fandom.com/api.php?" +
    new URLSearchParams({
      origin: "*",
      action: "parse",
      page: properPageName,
  ...
  return fetch(url)
    ...
    .then((html) => {
      ...
      const priceRange = $(html)
        .find("aside.portable-infobox div[data-source='price']")
        .children()
        .text();
      const maxPrice = parseInt(
        priceRange.split(" to ")[1].replaceAll(",", "")
      ); // parseInt doesn't handle separators
...
async function ensureItemsHaveMaxPriceSet() {
  ...
  for (let item of items.values()) {
    ...
    if (item.maxPrice !== undefined) continue;
  ...
  if (shouldSave) saveItemsToLocalStorage();
}

function formatTextListItem(format, item) {
  return format
    .replaceAll("{{name}}", item.getHumanReadableName())
...
function calculateTotalPrice() {
  ...
  for (let item of itemsSortedDescending) {
    let [itemTotalPrice, equation, error, warning] =
      item.calculateTotalPrice(true); // ignore custom values
...
function updateTotalPrice() {
  ...
  // hasn't loaded yet
  if (!priceCalculationItem) return;

  const totalSelectedPriceInItems = +(
    totalSelectedPrice / priceCalculationItem.maxPrice
  ).toFixed(2); // the unary + converts it back to a number, removing trailing zeroes
  const totalSelectedPriceInItemsFormatted = getLocaleString(
  ...
  if (getIsPriceShownInScreenshot()) {
    if (!isInPriceCalculationMode && shouldShowTotalInNormalMode) {
      ...
      const totalPriceInItems = +(
        totalPrice / priceCalculationItem.maxPrice
      ).toFixed(2); // the unary + converts it back to a number, removing trailing zeroes
      ...
      let totalPriceHTML = `${totalPriceFormatted}<img src="${coinImageUrl}" style="width: 14px; height: 14px;"><span style="display: inline-block; width: 10px;"></span>${totalPriceInItemsFormatted}<img src="${
        priceCalculationItem.url
      }" style="width: 14px; height: 14px;"><span style="display: inline-block; width: 10px;"></span>(${itemCountFormatted} item${
        itemCount === 1 ? "" : "s"
      })`;
      ...
      if (totalPriceInItemsList.length > 1) {
        ...
        for (const item of totalPriceInItemsList) {
          totalPriceHTML += `${item.totalPriceInItems}<img src="${item.url}" style="width: 14px; height: 14px;"><span style="display: inline-block; width: 10px;"></span>`;
        }

        totalPriceHTML += `(${itemCountFormatted} item${
          itemCount === 1 ? "" : "s"
        })`;
      }
...
function setSelectedState(item, cell, isSelected) {
  ...
  cellSelector
    .find(".quantityLabel")
    .css("opacity", isSelected && item.customQuantity !== undefined ? 0.5 : 1);
  cellSelector
    .find(".priceLabel")
    .css(
      "opacity",
      isSelected && item.customPriceOrMultiplier !== undefined ? 0.5 : 1
    );
}

function setSelectedStateAll(items, cells, isSelected) {
  for (let item of items) item.isSelected = isSelected;
...
async function prepareAllItemNames() {
  ...
  const prepared = [];
  for (let itemName of itemNames) prepared.push(fuzzysort.prepare(itemName));
...
async function getAllItemNames() {
  const fetchPortion = (pageName) => {
    ...
    return fetch(url)
      ...
      .then((images) =>
        images.map((image) => image.split(".png")[0].replaceAll("_", " "))
      );
  };
  ...
  return productNames
    .concat(cropNames, animalProductNames, suppliesNames)
    .filter((name) => !nameBlacklist.has(name))
    .concat([...customItemNames.values()]);
}
...
function updateFuzzyMatches(itemInput, fuzzyMatchesHolder) {
  ...
  if (
    (itemInput === itemNameInput && getIsInPriceCalculationMode()) ||
    !preparedItemNames
  )
    return;

  const matches = fuzzysort.go(itemInput.val(), preparedItemNames, {
    limit: 10,
  });
  ...
  for (let match of matches) {
    ...
    button.tabIndex = -1;
    button.innerHTML = fuzzysort.highlight(
      match,
    ...
    $(button).on(
      ...
      (event, customParams) => {
        ...
        if (!customParams || !customParams.usedKeyboard)
          // don't want to do this if the user selected a match using the keyboard via 1-9,0
          ...
          $("*").one("mouseup.fuzzyMatchClick", (e) => {
            if (
              shouldFocusQuantityOnAutocomplete &&
              itemInput === itemNameInput
            )
              itemQuantityInput.trigger("select");
            else itemInput.trigger("focus");
            e.stopPropagation();
...
function rescaleScreenshotRegion() {
  ...
  const heuristicFactor =
    document.documentElement.clientWidth / ((itemsPerRow + 1) * 110); // estimated width of table with all items in row filled in
  const actualFactor =
    (0.9 * document.documentElement.clientWidth) / screenshotRegion.width(); // actual calculated width of table (including bottom text)
...
function getSelectedItemCount() {
  let count = 0;
  for (let item of [...items.values()]) {
    if (item.isSelected) count += item.customQuantity ?? item.quantity;
  }
...
function getTotalItemCount() {
  let count = 0;
  for (let item of [...items.values()]) count += item.quantity; // ignore custom quantities
...
function updateAbbreviation(
  ...
  if (oldAbbreviation !== newAbbreviation) removeAbbreviation(oldAbbreviation);
  else saveAllToLocalStorage(); // removeAbbreviation() handles saving when called; no need to do it twice in that case
}
...
function exportAll() {
  let blob = new Blob([JSON.stringify(JSON.stringify(localStorage))], {
    type: "application/json",
  }); // need to do it twice to get double quotes properly escaped
...
const excludeFromItemLists = [
  "activeItemList",
  "itemLists",
  "includeSettingsInItemList",
  "copyCurrentItemsFromItemList",
  "lastUsedVersion",
  "v2.14_backup",
];
function createItemListObject(shouldIncludeItems) {
  ...
  if (shouldIncludeSettingsInItemList) {
    ...
    obj = { ...localStorage };
    for (let key of excludeFromItemLists) delete obj[key];
  }
...
function createNewItemList(name) {
  const obj = createItemListObject(
    shouldCopyCurrentItemsFromItemList || name === "Default"
  );
...
function loadItemList() {
  ...
  for (let key of Object.keys(obj)) localStorage[key] = obj[key];
...
function loadAllFromLocalStorage() {
  ...
  if (sItems) {
    ...
    for (let i in kvps) kvps[i][1] = Object.assign(new Item(), kvps[i][1]);
    items = new Map(kvps);
  }

  const sIncludeSettingsInItemList =
    (localStorage.getItem("includeSettingsInItemList") ?? "true") === "true"; // default to true
  ...
  const sCopyCurrentItemsFromItemList =
    (localStorage.getItem("copyCurrentItemsFromItemList") ?? "false") ===
    "true"; // default to false
  shouldCopyCurrentItemsFromItemList = sCopyCurrentItemsFromItemList;
  copyCurrentItemsFromItemListCheckBox.prop(
    "checked",
    sCopyCurrentItemsFromItemList
  );
  ...
  for (let name of itemLists.keys()) {
    ...
    itemListDropdown.append(option);
    if (activeItemList === name) $(option).prop("selected", true);
  }
  if (!itemLists.has("Default")) createNewItemList("Default");
  ...
  createItemListButton.prop(
    "disabled",
    itemLists.has(createItemListInput.val())
  );
  ...
  const sItemsPerRow =
    localStorage.getItem("itemsPerRow") ??
    Math.min(Math.floor(document.documentElement.clientWidth / 110), 8); // default up to 8 (however much fits; the exact calculation for the width a cell takes up is 8 + ct*100 + (ct-1)*10  AKA  110*ct - 2, but I rounded it slightly)
  ...
  textListSeparatorSelectedRadio = parseInt(
    localStorage.getItem("textListSeparatorSelectedRadio") ?? 0
  ); // don't really need to do this, but it would be best to treat it as an integer/number
  const sTextListCustomSeparator =
    localStorage.getItem("textListCustomSeparator") ?? "";
  textListSeparatorCustomRadio.val(sTextListCustomSeparator);
  textListCustomSeparatorInput.val(sTextListCustomSeparator);

  const sTextListFormat =
    localStorage.getItem("textListFormat") ??
    "{{quantity}} {{name}} ({{price}})"; // default format on right
  textListFormatInput.val(sTextListFormat);

  const sPriceCalculationItem =
  ...
  const sShowPriceInScreenshot =
    (localStorage.getItem("showPriceInScreenshot") ?? "true") === "true"; // default to true; localStorage only uses strings, so need to make sure to compare to "true" not true
  ...
  const sShowTotalInNormalMode =
    (localStorage.getItem("showTotalInNormalMode") ??
      (sShowPriceInScreenshot ? "true" : "false")) === "true"; // default to true so long as show price in screenshot is also enabled; localStorage only uses strings, so need to make sure to compare to "true" not true
  shouldShowTotalInNormalMode = sShowTotalInNormalMode;
  showTotalInNormalModeCheckBox.prop("checked", sShowTotalInNormalMode);

  const sHidePriceOrMultiplier =
    (localStorage.getItem("hidePriceOrMultiplier") ?? "false") === "true"; // default to false
  ...
  const sDefaultPriceOrMultiplier =
    localStorage.getItem("defaultPriceOrMultiplier") ?? "5x"; // default to 5x
  ...
  const sRefocusNameOnSubmit =
    (localStorage.getItem("refocusNameOnSubmit") ?? "true") === "true"; // default to true // TODO -- is this a good idea (default: true)?
  shouldRefocusNameOnSubmit = sRefocusNameOnSubmit;
  refocusNameOnSubmitCheckBox.prop("checked", sRefocusNameOnSubmit);

  const sFocusQuantityOnAutocomplete =
    (localStorage.getItem("focusQuantityOnAutocomplete") ?? "true") === "true"; // default to true // TODO -- is this a good idea (default: true)?
  shouldFocusQuantityOnAutocomplete = sFocusQuantityOnAutocomplete;
  focusQuantityOnAutocompleteCheckBox.prop(
    "checked",
    sFocusQuantityOnAutocomplete
  );

  const sIgnoreLocale =
    (localStorage.getItem("ignoreLocale") ?? "false") === "true"; // default to false
...
function saveAllToLocalStorage() {
  ...
  localStorage.setItem(
    "includeSettingsInItemList",
    shouldIncludeSettingsInItemList
  );
  localStorage.setItem(
    "copyCurrentItemsFromItemList",
    shouldCopyCurrentItemsFromItemList
  );
  ...
  localStorage.setItem(
    "abbreviationMapping",
    JSON.stringify([...abbreviationMapping])
  );
  ...
  localStorage.setItem(
    "textListSeparatorSelectedRadio",
    textListSeparatorSelectedRadio
  );
  localStorage.setItem(
    "textListCustomSeparator",
    textListCustomSeparatorInput.val()
  );
  ...
  localStorage.setItem(
    "showPriceInScreenshot",
    showPriceInScreenshotCheckBox.prop("checked")
  ); // TODO -- I should make my use of a separate variable vs using the checkbox directly consistent
...
function saveItemsToLocalStorage() {
  localStorage.setItem(
    ...
    JSON.stringify([...items], (key, value) =>
      Item.fieldsToOmitFromLocalStorage.has(key) ? undefined : value
    )
  );
}
...
const changelog = new Map([
  ...
  [
    "v1.2",
    `Forgot to manually change the size of the coin image (in relation to #1).`,
  ],
  [
    "v1.1",
    `Fixes the weird issue with images yielding a 404 only on github pages.`,
  ],
  [
    "v1.0",
    `Main features are:
...
function setUpChangelog() {
  ...
  changelogOverlay.inner.append(
    changes.flatMap((elem) => [elem, document.createElement("hr")]).slice(0, -1)
  );
}
...
function handleVersionChange() {
  ...
  if (sLastUsedVersion === latestVersion) return;
